...
// task 11, design §9): the plain interactive codex resume of one Session
// in the workflow Worktree, inheriting the caller's terminal. It is only
// available when the binding's NativeInteractiveResume capability is
// set; the argv is explicit and carries no bypass flag.
func (a *Adapter) InteractiveResume(ctx context.Context, session agent.ProviderSessionID, cwd string) (agent.InteractiveResumeSpec, error) {
	...
	if cwd != "" && !filepath.IsAbs(cwd) {
		return agent.InteractiveResumeSpec{}, model.InvalidInputFault("codex working directory must be an absolute path")
	}
