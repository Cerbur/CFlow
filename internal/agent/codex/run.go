// One supervised codex run (design 14.1/14.3): the run reads the
// supervisor's bounded framed pipeline, decodes each stdout frame
// through the dialect, and drops stderr (bounded in the supervisor,
...
// run is one supervised codex process event stream. established and
	...
	for {
		...
		case ev, ok := <-r.ch:
			...
			case process.EventFrameOut:
				...
				if skip {
					continue // known unmapped codex frames pass over silently
				}
			...
			case process.EventOverflowOut:
				...
				return agent.Event{}, &agent.ProtocolError{
					...
					Message: "codex stdout exceeded the bounded stream budget",
				}
			default:
...
func (r *run) afterReap(ctx context.Context) (agent.Event, error) {
	...
	return agent.Event{}, &agent.ProcessCrash{
		ExitCode: exit.Code,
		Message:  fmt.Sprintf("codex %s (code %d) without a terminal structured event", factText(exit.Fact), exit.Code),
	}
}
