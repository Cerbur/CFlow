...
// Codex Adapter tests (brief Steps 2-3): the verbatim
...
const capturedSessionID = "0197f1c1-9c6e-7a00-a000-000000000001"
...
func codexBinding(t *testing.T) agent.ProviderBinding {
	t.Helper()
...
// stubCodexOnPath creates a codex stub executable in a temp dir and puts
// that dir alone on PATH, so the adapter's real exec.LookPath resolution
// finds it deterministically. The Fake Supervisor never executes it; its
// content only feeds the executable hash facts.
...
// schemaFile creates the managed immutable schema file the adapter
// requires (the Application materializes it in production; the test
// materializes it here).
func schemaFile(t *testing.T) string {
	t.Helper()
	p := filepath.Join(t.TempDir(), "plan-envelope.json")
...
// a recording wrapper, with a stub codex executable on PATH and a managed
// schema file. probe is the 1-based supervisor start index the next
// Detect's version probe will occupy (advanced by detectIn).
type harness struct {
	ad       agent.Adapter
...
func newHarness(t *testing.T, binding agent.ProviderBinding) *harness {
	...
	ad := codex.New(rec, binding)
	return &harness{ad: ad, fake: fa, rec: rec, codexDir: filepath.Dir(stubCodexOnPath(t)), schema: schemaFile(t)}
}
...
// scriptFrames emits every non-empty fixture line as one stdout JSONL
// frame on start startIdx, then exits the group with code. When exitCode
// is -1 the group is left running (callers cancel it themselves).
...
// startRun launches a codex Start through the adapter (the run is the
// first supervisor start; no Detect probe is involved).
func (h *harness) startRun(t *testing.T, purpose model.AgentPurpose, input codex.Input) agent.Run {
	t.Helper()
...
// newCodexRuntime builds a Runtime wired to the codex adapter, with a
// fixed Clock, a fresh sequential ID source, the test redaction registry,
// and a fresh evidence root.
func newCodexRuntime(t *testing.T, ad agent.Adapter) *agent.Runtime {
	t.Helper()
	reg, err := agent.LoadProviderRegistry()
	requireNoError(t, err)
	rt, err := agent.NewRuntime(agent.RuntimeOptions{
		...
		Adapters:    map[string]agent.Adapter{"codex": ad},
	})
	requireNoError(t, err)
...
// fixtureStart builds a plain codex Start request.
func fixtureStart(h *harness) agent.StartRequest {
	return agent.StartRequest{
		...
		Input:    codex.Input{SchemaPath: h.schema},
		CWD:      "/Users/dev/worktrees/task-1",
	}
}
...
func fixtureCodexStart() codex.StartRequest {
	return codex.StartRequest{
		SchemaPath: "/managed/evidence/schemas/plan-envelope.json",
		Worktree:   "/worktrees/task-1",
		Prompt:     "Implement the search feature.",
	}
}

// TestStartArgvContainsNoBypassFlags (brief Step 2, verbatim): Start argv
// is exactly exec --json --output-schema <file> -C <worktree> - and can
// never contain danger, bypass, or ignore flags.
...
// TestStartArgvOptionalModel: the approved optional --model appears only
// when the typed request carries one.
func TestStartArgvOptionalModel(t *testing.T) {
	req := fixtureCodexStart()
	req.Model = "o3"
	argv := codex.StartArgv(req)
	requireExactArgs(t, argv, "exec", "--json", "--output-schema", req.SchemaPath, "-C", req.Worktree, "--model", "o3", "-")
	requireAbsentArgs(t, argv, "--dangerously-bypass-approvals-and-sandbox", "--skip-git-repo-check", "--ignore-user-config")
}
...
func TestResumeArgvShape(t *testing.T) {
	...
	argv := codex.ResumeArgv(req)
	requireExactArgs(t, argv, "exec", "resume", "--json", "--output-schema", req.SchemaPath, capturedSessionID, "-")
...
func TestCapturedHelpFixturesConfirmBinding(t *testing.T) {
	...
	for _, want := range []string{"--json", "--output-schema", "-C, --cd"} {
		if !strings.Contains(execHelp, want) {
			t.Fatalf("captured exec help must confirm %q", want)
		}
	}
	resumeHelp := readFixture(t, "resume-help.txt")
	for _, want := range []string{"[SESSION_ID] [PROMPT]", "--json", "--output-schema"} {
		if !strings.Contains(resumeHelp, want) {
			t.Fatalf("captured resume help must confirm %q", want)
		}
	}
	if strings.Contains(resumeHelp, "-C, --cd") {
		t.Fatal("captured resume help must not accept -C/--cd (the binding documents this incompatibility)")
	}
}
...
// through stdin, and an explicit safe env (HOME and PATH only): parent
// values never leak into the child.
func TestStartProcessSpecIsExact(t *testing.T) {
	...
	r := h.startRun(t, agent.PurposeImplementer, codex.Input{SchemaPath: h.schema})
	h.scriptFrames(t, 1, readFixture(t, "start-valid.jsonl"), 0)
	_ = collect(t, r)

	spec := h.rec.specAt(0)
	requireExactArgs(t, spec.Args, "exec", "--json", "--output-schema", h.schema, "-C", "/Users/dev/worktrees/task-1", "-")
	requireAbsentArgs(t, spec.Args, "--dangerously-bypass-approvals-and-sandbox", "--skip-git-repo-check", "--ignore-user-config")
	if spec.Executable != filepath.Join(h.codexDir, "codex") {
		t.Fatalf("executable = %q, want the PATH-resolved codex stub", spec.Executable)
	}
	...
	requireNoError(t, err)
	if string(prompt) != "Implement the search feature." {
		t.Fatalf("stdin prompt = %q, want the request prompt verbatim", prompt)
	}
	if len(spec.Env) != 2 || spec.Env["PATH"] != h.codexDir || spec.Env["HOME"] == "" {
...
// TestResumeProcessSpecIsExact: Resume launches exec resume with the
// session id and no -C; the process cwd still carries the request's CWD.
func TestResumeProcessSpecIsExact(t *testing.T) {
	...
	spec := h.rec.specAt(0)
	requireExactArgs(t, spec.Args, "exec", "resume", "--json", "--output-schema", h.schema, capturedSessionID, "-")
	requireAbsentArgs(t, spec.Args, "-C", "--dangerously-bypass-approvals-and-sandbox", "--skip-git-repo-check", "--ignore-user-config")
	if spec.Dir != "/Users/dev/worktrees/task-1" {
...
// TestDialectMalformedFrameFailsClosed: a non-JSON stdout frame under
// --json stops the stream with a protocol violation carrying the raw
// frame; events read before the failure are preserved.
func TestDialectMalformedFrameFailsClosed(t *testing.T) {
	...
	if !bytes.Contains(pe.Frame, []byte("this is not a jsonl frame")) {
...
// codex dialect (fixture protocol-invalid.jsonl) stops the run with a
...
// TestAdapterStartRequiresManagedSchema: the adapter fails closed before
// any process launch when the request carries no proven managed schema
// file (the argv contract always passes --output-schema).
func TestAdapterStartRequiresManagedSchema(t *testing.T) {
	h := newHarness(t, codexBinding(t))
	_, err := h.ad.Start(context.Background(), agent.StartRequest{
		Purpose:  agent.PurposePlanner,
		Provider: "codex",
		Prompt:   "Plan.",
		Input:    map[string]any{"requirement": "search"},
		CWD:      "/Users/dev/worktrees/task-1",
	})
	assertFaultCode(t, err, model.CodeInvalidInput)
	if h.rec.count() != 0 {
		t.Fatal("an unproven schema must fail before any process launch")
	}
}
...
func TestAdapterCapabilityGates(t *testing.T) {
	b := codexBinding(t)
	b.StartCapabilities = []string{"jsonl_events"}
	h := newHarness(t, b)
	_, err := h.ad.Start(context.Background(), agent.StartRequest{
		Purpose:  agent.PurposePlanner,
		Provider: "codex",
	...
	b2 := codexBinding(t)
	b2.ResumeCapabilities = []string{"jsonl_events"}
	h2 := newHarness(t, b2)
	_, err = h2.ad.Resume(context.Background(), agent.ResumeRequest{
		...
		Provider:          "codex",
		Prompt:            "Continue.",
		Input:             codex.Input{SchemaPath: h2.schema},
		CWD:               "/Users/dev/worktrees/task-1",
	})
	assertFaultCode(t, err, model.CodeProviderProtocolUnsupported)
	if h2.rec.count() != 0 {
...
// ---------------------------------------------------------------------------
// Runtime integration: the codex dialect plugs into the unified pipeline
// ---------------------------------------------------------------------------

// TestCodexAdapterPlugsIntoRuntimePipeline: a Runtime wired to the codex
// adapter validates the protocol sequence, redacts every text-bearing
// field, and settles the Session from the terminal structured event.
func TestCodexAdapterPlugsIntoRuntimePipeline(t *testing.T) {
	h := newHarness(t, codexBinding(t))
...
// TestCodexEventTextRedactedThroughPipeline: a provider token in an
// assistant event is redacted before any output consumer sees it.
func TestCodexEventTextRedactedThroughPipeline(t *testing.T) {
	h := newHarness(t, codexBinding(t))
	rt := newCodexRuntime(t, h.ad)
...
// session the Runtime does not know fails closed before any codex process
// is launched.
...
// TestResumeReaffirmsBoundSession: a resume stream whose start event
// re-affirms the resumed session completes; the runtime's bound check is
// exercised through the codex dialect.
...
func TestDetectSupportedCapturedBinding(t *testing.T) {
	...
	wantHash := sha256Hex(mustReadFile(t, filepath.Join(h.codexDir, "codex")))
	if inst.ExecutableSHA256 != wantHash {
		t.Fatalf("executable hash = %q, want %q", inst.ExecutableSHA256, wantHash)
	}
	if inst.DialectID != "cflow.dialect.codex-jsonl.v1" {
		t.Fatalf("dialect = %q", inst.DialectID)
	}
	if inst.RegistryRevision != codexBinding(t).Revision {
...
// TestDetectMissingExecutable: no codex executable on PATH yields the
...
func TestDetectMissingExecutable(t *testing.T) {
	...
	ad := codex.New(sup, codexBinding(t))
...
// parsed as the codex CLI version yields INCOMPATIBLE_PROTOCOL.
func TestDetectIncompatibleProtocol(t *testing.T) {
	for _, scripted := range []string{"", "some other tool v3", "codex-cli banana"} {
...
func TestDetectExecutableHashDrift(t *testing.T) {
	...
	fa, sup := process.NewFakeSupervisor()
	rec := &recordingSupervisor{Supervisor: sup}
	ad := codex.New(rec, codexBinding(t))
...
func TestDetectRealCodexCapturedBinding(t *testing.T) {
	...
	if err != nil {
		t.Skip("codex is not installed; opt-in smoke detection skipped")
	}
...
// Task 16): the codex typed input carries the immutable Context Bundle
...
func TestTypedInputCarriesContextBundleRef(t *testing.T) {
	withRef := codex.Input{SchemaPath: "/schemas/coding-output.json", Model: "gpt-5",
		ContextBundleRef: "/evidence/sessions/lost1/bundle-1.json"}
	...
	fresh, err := json.Marshal(codex.Input{SchemaPath: "/schemas/coding-output.json", Model: "gpt-5"})
	requireNoError(t, err)
...
// exactly `codex resume <session-id>` in the workflow Worktree — no
// bypass flag, no -C.
func TestInteractiveResumeArgvShape(t *testing.T) {
	...
	if ia, ok := ad.(agent.InteractiveAdapter); ok {
		...
		if spec.Capability {
			...
			requireAbsentArgs(t, spec.Args, "--dangerously-bypass-approvals-and-sandbox", "-C", "--skip-git-repo-check")
