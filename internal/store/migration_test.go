...
func TestMigrationFromFreshDatabase(t *testing.T) {
	...
	rows := schemaRows(t, s)
	if len(rows) != 2 {
		t.Fatalf("schema_migrations = %d rows, want 2", len(rows))
	}
...
func TestMigrationAppliesForwardChainFromV1(t *testing.T) {
	...
	backupDir := filepath.Join(filepath.Dir(path), "backups", "db", "cflow-002-cleanup-apply")
	...
	if mf.SourceVersion != 1 || mf.TargetVersion != 2 || mf.CflowVersion != "2.0.0" {
	...
	reg := migrations()
	if len(mf.Migrations) != 1 || mf.Migrations[0].ID != reg[1].ID || mf.Migrations[0].SHA256 != reg[1].SHA256 {
...
func TestMigrationPathMissingFailsClosed(t *testing.T) {
	...
	path := fixtureWithVersionMutator(t, func(tx *sql.Tx, _ *sql.DB) {
		...
		if _, err := tx.Exec(`INSERT INTO schema_migrations
			(version, migration_id, migration_sha256, cflow_version, applied_at)
			VALUES (2, 'cflow-002-cleanup-apply', ?, '0.0.0-dev', ?)`, sha256Of(migrations()[1].SQL), nowText()); err != nil {
...
func TestCrashAfterBackupManifestRetriesIdempotently(t *testing.T) {
	...
	defer s2.Close()
	if got := userVersion(t, s2); got != 2 {
		t.Fatalf("user_version = %d, want 2", got)
	}
	rows := schemaRows(t, s2)
	if len(rows) != 2 || rows[1].ManifestSHA256 != sha256Of(string(manifestBody)) {
...
func TestCrashAfterMigrationCommitRecognizesCompletion(t *testing.T) {
	...
	if rows := schemaRows(t, s2); len(rows) != 2 {
		t.Fatalf("rows = %d, want 2 (no re-run)", len(rows))
	}
}
...
func TestMigrationConcurrentOpensMigrateExactlyOnce(t *testing.T) {
	...
	rows := schemaRows(t, s)
	if len(rows) != 2 {
		t.Fatalf("rows = %d, want 2 (migrated exactly once)", len(rows))
	}
	if got := userVersion(t, s); got != 2 {
		t.Fatalf("user_version = %d, want 2", got)
	}
